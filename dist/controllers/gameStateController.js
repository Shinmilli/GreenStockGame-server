...
exports.gameState = exports.forceNextPhase = exports.resetGame = exports.startGame = exports.getGameState = void 0;
...
const getGameState = async (req, res) => {
    try {
        ...
        const { message, statusCode } = (0, errorHandler_1.handleControllerError)(error, '게임 상태 조회');
...
const startGame = async (req, res) => {
    try {
        ...
        const { message, statusCode } = (0, errorHandler_1.handleControllerError)(error, '게임 시작');
        res.status(statusCode).json({ message });
    }
};
...
const resetGame = async (req, res) => {
    try {
        ...
        // 모든 팀 잔액 및 점수 초기화
        await models_1.prisma.team.updateMany({
            data: {
        ...
        // 모든 보유 주식 삭제
        await models_1.prisma.holding.deleteMany({});
        // 모든 거래 기록 삭제
        await models_1.prisma.transaction.deleteMany({});
        res.json({
        ...
        const { message, statusCode } = (0, errorHandler_1.handleControllerError)(error, '게임 초기화');
        res.status(statusCode).json({ message });
    }
};
exports.resetGame = resetGame;
const forceNextPhase = async (req, res) => {
    try {
        if (!gameState.isActive) {
        ...
        const { message, statusCode } = (0, errorHandler_1.handleControllerError)(error, '페이즈 강제 이동');
        res.status(statusCode).json({ message });
    }
};
exports.forceNextPhase = forceNextPhase;
// 내부 함수들
...
async function activateRoundEvents(roundNumber) {
    try {
        // 해당 라운드의 뉴스 이벤트들 조회
